...
import com.example.bookworld.database.async.favorite.GetFavoritesAsyncTask;
import com.example.bookworld.database.db.AppData;
import com.example.bookworld.database.db.DbResponse;
import com.example.bookworld.databinding.ActivityFavoriteBinding;
...
public class FavoriteListActivity extends AppCompatActivity implements FavoriteWatchActionListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(view);
        loadAndShowFavoriteBooks();
    }

    private void loadAndShowFavoriteBooks() {
        GetFavoritesAsyncTask task = new GetFavoritesAsyncTask(this, Action.GET_FAVORITES_ACTION,
                new DbResponse<List<Book>>() {
                    ...
                    public void onError(Error error) {
                        Toast.makeText(FavoriteListActivity.this, error.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
    ...
    public void onItemClicked(Book book) {
        ...
        Intent intent = new Intent(FavoriteListActivity.this, BookDetailsActivity.class);
        startActivity(intent);
    }

    @Override
    public void onDeleteClicked(Book book, int position) {
        UserBookCrossRef userBookCrossRef = new UserBookCrossRef(AppData.getCurrentUser().getId(), book.getBookId());
        FavoriteCudAsyncTask task = new FavoriteCudAsyncTask(getApplicationContext(), Action.DELETE_ACTION, new ResultListener<UserBookCrossRef>() {
            ...
            public void onResult(Result<UserBookCrossRef> response) {
                ...
                if ((response == null) || (error != null)) {
                    ...
                    Toast.makeText(FavoriteListActivity.this, errorMsg, Toast.LENGTH_LONG).show();
                    return;
                }
                adapter.deleteItem(position);
                Toast.makeText(FavoriteListActivity.this, getString(R.string.message_successful_remove), Toast.LENGTH_LONG).show();
            }
        });
